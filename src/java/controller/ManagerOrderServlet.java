...
public class ManagerOrderServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        DAO dao= new DAO();
         int endPage = Helper.getEndp(dao.getTotalOrder());
        List<Order> list= dao.pagingOrder(indexpage);
           if(!list.isEmpty()){
             if(indexpage==endPage){
                 request.setAttribute("begin", dao.getTotalOrder());
             }else{
                 int begin= list.size()*indexpage;
             request.setAttribute("begin", begin);
             }
             request.setAttribute("total", dao.getTotalOrder());
         }
